...
onMounted(async () => {
  try {
    ...
    validateData<DPIA>(dpiaFormValidation, (validData) => {
...
<template>
  <div class="dpia-container">
    <div v-if="isLoading" class="loading">
      ...
      <p>Loading DPIA tasks...</p>
    </div>

    <div v-else-if="error" class="error">
      <h2>Validation Error</h2>
      <pre class="error-message">{{ error }}</pre>
      <p class="error-help">Please check the DPIA JSON data structure and ensure it matches the expected format.</p>
    </div>

    <div v-else class="content">
      <header>
        <h1>DPIA Tasks</h1>
        <p class="summary">{{ tasks.length }} task{{ tasks.length !== 1 ? 's' : '' }} found</p>
      </header>
