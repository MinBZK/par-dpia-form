...
const isTaskGroup = computed(() => props.task.type?.includes('task_group') || false)

const hasChildTasks = computed(() => (props.task.childrenIds?.length || 0) > 0)

function getTaskNameById(id: string): string {
  return taskStore.taskById(id).task
}
</script>

<template>
  <div class="rvo-layout-margin-vertical--s">
    <h1 class="utrecht-heading-1">{{ task.id }}. {{ task.task }}</h1>
