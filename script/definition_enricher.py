    ...
    for task in tasks:
        ...
        if level == 0:
            if "description" in task_copy and isinstance(task_copy["description"], str):
                task_copy["description"] = inject_terms(task_copy["description"], term_map)
        ...
        if is_checkbox_option and "options" in task_copy and isinstance(task_copy["options"], list):
            options_copy = []
...
class DefinitionEnricher:
    ...
    def enrich_and_export(self, source_path, begrippen_yaml_path, output_path):
        ...
        print(f"Begrippenkader processed:")
        print(f"- {len(hoofdtermen)} main terms")
        print(f"- {len(alt_spellingen)} alternative spellings")  
        print(f"- {len(alt_termen)} alternative terms")
        print(f"- {len(term_map)} terms in total")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Converteer DPIA.yaml of prescan_DPIA.yaml naar JSON met begrippenkader injecties.')
    parser.add_argument('--source', required=True, 
                        help='Pad naar bron YAML bestand (DPIA.yaml of prescan_DPIA.yaml)')
    parser.add_argument('--definitions', required=True,
                        help='Pad naar begrippenkader-dpia.yaml bestand')
