...
def extract_task_info(task, parent_path=""):
    ...
    task_type = ", ".join(task.get('type', [])) if isinstance(task.get('type', []), list) else task.get('type', "")
...
Example usage:
  ...
  python generate_md_table_questions.py \\
    ...
    parser.add_argument("--output", help="Path to the output markdown file (default: derived from source filename)")
