...
const props = defineProps<{
  // Absolute heading levels available in this editor (e.g. [2,3,4,5,6]); shown in
  // the dropdown as "Kop 1".."Kop N". The active block's level (or null for a
  // paragraph) drives the dropdown's current value.
...
interface ToolbarButton {
  ...
  // tooltip. Omitted for commands without a shortcut (divider, link).
...
// The dropdown shows "Kop 1".."Kop N" for the available levels; its value is the
// active block's level, or '' for a paragraph. Changing it emits the chosen level
// (null = back to paragraph).
function onHeadingChange(event: Event) {
  const value = (event.target as HTMLSelectElement).value
  emit('heading', value === '' ? null : Number(value))
}

// The toolbar is one tab stop (roving tabindex): index 0 is the heading dropdown,
// 1..N the buttons. Left/Right/Home/End move across all controls. The dropdown
// keeps its native Up/Down for cycling options (those keys fall through below).
...
function onKeydown(event: KeyboardEvent) {
  ...
  const last = buttons.length // controls = [select, ...buttons], so the last index is buttons.length
  ...
  const elements = (event.currentTarget as HTMLElement).querySelectorAll('select, button')
...
<template>
  <div class="markdown-toolbar" role="toolbar" aria-label="Tekstopmaak" @keydown="onKeydown">
    <select class="utrecht-select utrecht-select--html-select markdown-toolbar__heading"
      aria-label="Kopniveau" :value="props.activeHeadingLevel ?? ''"
      :tabindex="activeIndex === 0 ? 0 : -1"
      @change="onHeadingChange" @focus="activeIndex = 0">
      <option value="">Gewone tekst</option>
      <option v-for="(level, index) in props.headingLevels" :key="level" :value="level">Kop {{ index + 1 }}</option>
    </select>
